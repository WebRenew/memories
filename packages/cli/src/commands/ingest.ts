...
function inferType(line: string): MemoryType {
  ...
  return "rule"; // Default to rule for instruction files
}
...
function extractMemories(content: string): { content: string; type: MemoryType }[] {
  ...
  for (const line of clean.split("\n")) {
    ...
    // Plain text lines (not headers, not empty, not too short)
...
export const ingestCommand = new Command("ingest")
  ...
  .argument("[source]", "Source to import from (cursor, claude, agents, copilot, windsurf, cline, roo, gemini, or file path)")
  .option("--type <type>", "Override type for all imported memories")
  .option("--dry-run", "Preview without importing")
  .option("--all", "Scan all known IDE rule file locations")
  .action(async (source: string | undefined, opts: { type?: string; dryRun?: boolean; all?: boolean }) => {
    try {
      ...
      if (opts.all) {
        ...
        if (known) {
          ...
          console.error(chalk.red("✗") + ` Unknown source "${source}". Valid: ${SOURCES.map((s) => s.name).join(", ")}, or a file path`);
          process.exit(1);
        }
      } else {
      ...
      for (const file of filesToProcess) {
        ...
        for (const mem of memories) {
          const type = (opts.type as MemoryType) ?? mem.type;
          if (opts.dryRun) {
      ...
      if (opts.dryRun) {
        console.log(chalk.dim("\n  Dry run — no memories imported. Remove --dry-run to import."));
      } else {
        console.log(chalk.green("\n✓") + ` Imported ${totalImported} memories`);
      }
    } catch (error) {
