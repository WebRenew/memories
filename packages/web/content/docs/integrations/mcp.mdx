...
**Recommended: stdio transport** — Add to `.cursor/mcp.json` in your project (or `~/.cursor/mcp.json` globally):
...
```json
{
  "mcpServers": {
    "memories": {
      "command": "npx",
      "args": ["-y", "@memories.sh/cli", "serve", "--api-key", "REPLACE_WITH_YOUR_API_KEY"]
    }
  }
}
```

This runs the MCP server locally and connects via stdio, which is more reliable than HTTP transport.

**Alternative: HTTP transport** — If you prefer not to run a local process:
