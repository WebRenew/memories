...
<Callout>
Scope model by surface: CLI uses `global` + git project scope. SDK uses `tenantId`
(security/database boundary), `userId` (end-user scope), and `projectId` (optional repo
context filter).
</Callout>
