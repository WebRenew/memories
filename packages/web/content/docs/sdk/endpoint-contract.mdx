...
- `qualityGate`: current/previous window eval snapshot with `status`, `canaryBlocked`, and detailed `reasons`
- `alarms`: generated safety alarms such as `HIGH_FALLBACK_RATE` and `GRAPH_EXPANSION_ERRORS`

`/api/sdk/v1/graph/rollout` now enforces a hard canary safety gate:

- `POST/PATCH mode=canary` returns `409` + `CANARY_ROLLOUT_BLOCKED` when quality regressions exceed thresholds.
- Successful rollout responses include `qualityGate` so clients can render gate state without calling status separately.
