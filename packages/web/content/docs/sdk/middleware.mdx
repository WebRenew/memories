...
`memoriesMiddleware()` is the headline feature of the SDK. It wraps any [Vercel AI SDK](https://ai-sdk.dev/) model so that rules and relevant memories are automatically injected into the system prompt — no tool calls, no prompt engineering.
...
[AI SDK](https://ai-sdk.dev/) middleware is composable. Stack `memoriesMiddleware()` with other middleware:
...
This is lighter than provider wrapping (Mem0's approach) and plays well with the rest of the [AI SDK](https://ai-sdk.dev/) ecosystem.
