...
- `auto` (default): uses `mcp` only when `baseUrl` ends with `/api/mcp`; otherwise uses `sdk_http`
...
const rotated = await client.management.keys.create({
  expiresAt: "2026-12-31T00:00:00.000Z",
})
await client.management.keys.revoke()

const mappings = await client.management.tenants.list()
await client.management.tenants.upsert({
  tenantId: "acme-prod",
  mode: "provision",
})
await client.management.tenants.disable("acme-prod")
```
