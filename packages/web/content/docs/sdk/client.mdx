...
If `tenantId` is provided and no mapping exists yet, the runtime can auto-provision a tenant Turso database (when server provisioning credentials are configured). This lets SaaS apps avoid manual tenant DB setup during first use.
